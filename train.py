...
def main(_run, _config, _log):
    ...
    log_loss = {'total_loss': 0, 'query_loss': 0, 'align_loss': 0, 'aux_loss': 0, 'b_loss': 0, 'ssp_loss': 0}
    ...
    for sub_epoch in range(n_sub_epochs):
        ...
        for _, sample in enumerate(train_loader):
            ...
            query_pred, align_loss, aux_loss, b_loss, ssp_loss = model(support_images, support_fg_mask, query_images, train=True)
            aux_loss = 0.3 * aux_loss
            ssp_loss = 0.3 * ssp_loss
            ...
            ssp_loss = ssp_loss.detach().data.cpu().numpy()
            ...
            _run.log_scalar('ssp_loss', ssp_loss)
            ...
            if (i_iter + 1) % _config['print_interval'] == 0:
                ...
                _log.info(f'step {i_iter + 1}: total_loss: {total_loss}, query_loss: {query_loss}, aux_loss: {aux_loss}'
                          f', b_loss: {b_loss}, ssp_loss: {ssp_loss}, align_loss: {align_loss}')
