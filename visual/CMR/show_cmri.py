...
class Visual:
    ...
    def __init__(self, model='BePMI', organ_name="LIVER"):
        pass
    ...
    def update_status(self, model='BePMI', organ_name="LIVER"):
        ...
        self.abd_ct_pth = "./init"
        self.out_path = f"./sample/CMR/{model}/"
        self.model = model
    ...
    def show1(self):
        qry_img_pth = os.path.join(self.abd_ct_pth, 'init', f"image_{self.organ['qry_patient_id']}.nii.gz")
        qry_gt_pth = os.path.join(self.abd_ct_pth, 'init', f"label_{self.organ['qry_patient_id']}.nii.gz")
        qry_pred_pth = os.path.join(self.abd_ct_pth, self.model,
                                    f"prediction_{self.organ['qry_patient_id']}_{self.organ['name']}.nii.gz")

        supp_img_pth = os.path.join(self.abd_ct_pth, 'init', f"image_{self.organ['supp_patient_id']}.nii.gz")
        supp_gt_pth = os.path.join(self.abd_ct_pth, 'init', f"label_{self.organ['supp_patient_id']}.nii.gz")
        ...
        supp_gt[supp_gt== 200] = 1
        supp_gt[supp_gt == 500] = 2
        supp_gt[supp_gt == 600] = 3
...
#     delete_specific_files(f"./sample/CMR/{method}/",".png")
#     time.sleep(1)
...
for method in methods:
    ...
    visual.update_status(method, "LV-MYO")
    visual.show1()
    visual.update_status(method, "LV-BP")
    visual.show1()
    visual.update_status(method, "RV")
    visual.show1()
