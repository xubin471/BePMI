...
import torch
import shutil
# shutil.rmtree("/home/su/xb/BePMI/visual/Prostate_MRI/sample/Prostate_MRI")
...
class Visual:
    ...
    def update_status(self, model='BePMI', organ_name="Bladder"):
        organ = {
            "Bladder": {
                "name": "Bladder",
                "label_id": 1,
        ...
        self.prostate_mri_pth = "./img"
        self.out_path = f"./sample/Prostate_MRI/{model}/"
        self.model = model

        os.makedirs(self.out_path, exist_ok=True)

    def show1(self):
        qry_img_pth = os.path.join(self.prostate_mri_pth, 'init', f"image_{self.organ['qry_patient_id']}.nii.gz")
        qry_gt_pth = os.path.join(self.prostate_mri_pth, 'init', f"label_{self.organ['qry_patient_id']}.nii.gz")
        qry_pred_pth = os.path.join(self.prostate_mri_pth, self.model,
                                    f"prediction_{self.organ['qry_patient_id']}_{self.organ['name']}.nii.gz")

        supp_img_pth = os.path.join(self.prostate_mri_pth, 'init', f"image_{self.organ['supp_patient_id']}.nii.gz")
        supp_gt_pth = os.path.join(self.prostate_mri_pth, 'init', f"label_{self.organ['supp_patient_id']}.nii.gz")
        ...
        os.rename(self.out_path + f"/{self.organ['name']}_patient{self.organ['qry_patient_id']}_slice{self.organ['qry_slice']}_pred.png",f"./sample/pred_{self.model}.png")
        os.rename(
            self.out_path + f"/{self.organ['name']}_patient{self.organ['qry_patient_id']}_slice{self.organ['qry_slice']}_gt.png",
            f"./sample/gt.png")
...
methods = ["CoW", "QNet_T", "BePMI", "GMRD_T"]

# methods = ["CoW","QNet","GMRD"]
for method in methods:
    print(f"======== Method {method} begin ===============")
    # visual.update_status(method, "Bladder")
    # visual.show1()
    visual.update_status(method, "Bone")
    visual.show1()
    # visual.update_status(method, "TZ")
    # visual.show1()
    # visual.update_status(method, "SV")
    # visual.show1()
